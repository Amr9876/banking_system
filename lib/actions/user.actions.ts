...
export const signUp = async ({ password, ...userData }: SignUpParams) => {
  ...
  try {
    ...
    const newUserFromDB = await database.createDocument(
      DATABASE_ID!,
      USER_COLLECTION_ID!,
      ID.unique(),
      {
    ...
    return parseStringify(newUserFromDB);
  } catch (error) {
    console.error("Error", error);
  }
};
...
export const exchangePublicToken = async ({
  ...
  try {
    ...
    const accResp = await plaidClient.accountsGet({
      access_token: accessToken,
    });

    const accountData = accResp.data.accounts[0];
    ...
    const processorTokenResp = await plaidClient.processorTokenCreate(req);
    const processorToken = processorTokenResp.data.processor_token;
    ...
    if (!fundingSourceUrl) throw new Error("funding source url is not found");
